...
function onSdpText(remoteVideo) {
  ...
  settings['conn'].send(JSON.stringify({command: "subscribe", room_id: room_id, username: username }));
...
function registSdp(sdp) {
  ...
  $.ajax({
    ...
    settings['conn'].send(JSON.stringify({command: "message", room_id: room_id, username: username }));
  }).fail(function(jqXHR, status, errorThrown ) {
  }).always(function(){
  });
}
